$(document).ready(function (){
	...
	function selectModalData(id, url){
		...
		$(id + ' .ifream').attr('src', serverUrl+url); 
// console.log($(id + ' .ifream'), $(id + ' .ifream').attr('src'))
	    $(id + ' .ifream').on('load',function(){
	    	// console.log($(id+' .refresh_modal'))
	...
	function createModal(id){
		return '<div class="modal" id="modal_'+id+'" data-backdrop="static" data-keyboard="false">'+
					...
					'<div class="modal-contents ui-widget-content">'+
						...
						'<iframe src="/" class="ifream"></iframe'
						'</div>'+
					'</div>'+
				'</div>'
	}
	function reloadModal (id, modalData) {
		$(id).on("shown.bs.modal", function () {
			...
			$(this).css({'z-index': 2000, top: '120px'})
	...
	function init_more_menu_li_hover_event (e, obj){
		...
		$('div[data-id=submenu_'+obj.attr('data-id')+']').css({top: top_position, left: posLeft, opacity:1, 'z-index':2002})
		$('div[data-id=submenu_'+obj.attr('data-id')+'] .sub_menu_header').html('<a><span>'+obj.children('div').children('a').html()+'</span></a>')
	...
	$.ajax({
		...
		success: function (data){
				...
				for (var i = 0; i < one.length; i++) {
					if(one[i].hasChild == 0){
							$(ul).append('<li  data-id="'+one[i].id+'">'+
											'<div ><img src="'+iconUrl+one[i].module_keyword+'.png" /></div>'+
											'<div class="menu-slide">'+
												'<a href="#" title="'+one[i].module_name+'" data-toggle="modal" data-target="#modal_' + one[i].id + '">'+
			...
			var sub = [];
			var data_sub = [];
			function createSubMenu(submenu, data){
				// console.log(submenu)
						...
						if(data[l].pid == submenu[i] ) {
							...
							if(data[l].hasChild == 0){
								$(divs).append(//'<li class="first-li">' +
										'<a href="#" title="'+data[l].module_name+'" data-toggle="modal" data-target="#modal_'+data[l].id+'">'+
											'<img src="'+iconUrl+data[l].module_keyword+'.png" />'+
								...
								data_sub.push(data[l].id); //data.splice(l, l+1)[0];
								// console.log('a',a)
							} else {
								...
								$(divs).append(
										'<div >'+
											...
											'<h5>'+data[l].module_name+'</h5>'+
										'</div>');
								var menu_id = data[l].id //data.splice(l, l+1)[0].id
								data_sub.push(data[l].id);
							    submenu.push(menu_id);
			...
			point(one);
			var arr = createSubMenu(submenu, data)
			while ( arr != [] && submenu.length != 0) {
				for(var c = 0; c < submenu.length; c++) {
					for(var k = 0; k < sub.length; k++) {
						if(sub[k] == submenu[c]) {
							submenu.splice(c, c+1)
						}
					}
				}
				for(var d = 0; d < data.length; d++) {
					for(var s = 0; s < data_sub.length; s++) {
						if(data[d].id == data_sub[s].id) {
							data.splice(d, d+1)
						}
					}
				}
				arr = createSubMenu(submenu, data)
			}
			// return;
			// var minWidth  =one[i].size.width/100*width
			// var minHeight  =one[i].size.height/100*height
	...
	$(document).on('click', '.text-show, .map-show, .map-text-show, .fix-data', function(){
		$('.sub-menu').attr('style', '')
	})
})
