...
var map,infowindow,currentInfo,gcFuel=[],gcDNotRpt=[],gcDon=[],gcDoff=[],TripsIn=[],TripsOut=[],allGeofences=[],LoadPolySites=[],markers=[] ,marker=[],DtripsIn=[],DtripsOut=[],DtripsLoadingTime=[],Dname=[],Dcategory=[],DuniqueId=[],Dstatus=[],DlastUpdate=[],DgroupId=[],groups=[],events=[],Etype=[],Paddress=[],Pspeed=[],positions=[],Platitude=[],Plongitude=[],myInfo=[],Pcourse=[],displayTable=[],nearestSite=[],nearestSiteLat=[],nearestSiteLng=[],nearestLocation=[],DlastSpeed=[],distance=[],DmaxSpeed=[],DmaxSpeedC=[],Ptotaldistance=[],Pstartdistance=[],PFuel=[],SFuel=[],StolenFuelQty=[],FillFuelQty=[],FuelTheftFilled=[],MarkerStatus , markerCurrentState=[],DFitness=[],DInsurance=[],DPermit=[],DRoadtax=[],DPollution=[];
MarkerStatus="";
...
success: function(returnedData) {
	...
	if(returnedData[i].lastupdate!=null){ 
       ...
       DPollution[returnedData[i].id] = returnedData[i].PollutionExpiry;
            ...
            if(returnedData[i].Fuel != undefined)
            ...
            <div class="navbar-header">
               <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1">
                ...
                <?php if(Session::get('loggedIn')==true) { ?>
                            ...
                            <li><a href="<?php echo URL; ?>vts"><img src="<?php echo URL; ?>public/Icon-header/Home.png" style="float: left;font-size:18px;margin-left:-5px;">&nbsp;HOME  </a></li>
                            ...
                            <?php if(Roles::handleRole("master/vehicle")>0){ ?>
                             ...
                             <?php if(Roles::handleRole("master/driver")>0){ ?>
                               ...
                               <!-- <li><a href="<?php// echo URL; ?>master/customermaster">CUSTOMER</a></li>-->
                                ...
                                <?php if(Roles::handleRole("master/group")>0){ ?>
                                ...
                                <?php } ?>
                                <?php if(Roles::handleRole("master/customer")>0){ ?>
                                ...
                                <?php } ?>
                                <?php if(Roles::handleRole("master/expense")>0){ ?>
                           ...
                           <?php if(Roles::handleRole("reports/historicPlayback")>0){ ?>
                                ...
                                <?php } ?>
                                 <?php if(Roles::handleRole("reports/fuel")>0){ ?>
                                ...
                                <?php } ?>
                                 <?php if(Roles::handleRole("reports/route")>0){ ?>
	...
	<li><div class='notification-comments'><?php //echo $value['event_type']."<br>"?></div></li>!-->
         ...
         <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><img src="<?php echo URL; ?>public/Icon-header/User.png" style="float: left;font-size:18px;margin-left:-5px;">&nbsp;<?php echo Session::get('username'); ?></a>
                           ...
                           <!--<li><a href="<?php echo URL; ?>login/logout"><i class="fa fa-user-circle" aria-hidden="true" style="font-size:24px;color:white"></i>&nbsp;<?php echo Session::get('username'); ?>[LOGOUT]</a></li>-->
							...
							<?php  }  else { ?>  
                         		<li class="social pull-right"><a href=""><img src="<?php echo URL; ?>public/Icon-header/Home.png" style="float: left;font-size:18px;margin-left:-5px;">&nbsp;HOME  </a></li>
 							<?php } ?>
						</ul>
            </div>
            <!-- /.navbar-collapse -->
		   ...
		   var GetIdle;
			...
			//	if(isDefined(Pspeed[positions[i]])){
				...
				if(travelled_distance!=0 && initialFuel !=0 && currentFuel !=0){
					...
					var tdSno = "",tdGroups = "",tdDeviceID = "",tdCurrentStatus = "",tdCategory = "",tdDname = "",tdLastUpdate = "",tdNearestSite = "",tdSpeed = "",tdMaxSpeed = "",tdIdleTime = "",tdOddometer = "",tdTravelledDistance = "",tdInitialFuel = "",tdFilled = "",tdConsumed = "",tdStolen = "",tdCurrentFuel = "",tdKMPL = "",tdAddress ="" , tdTxt = "black",tdFitness ="",tdInsurance="",tdPermit="",tdRoadtax="",tdPollution="",tdTripsIn="",tdTripsOut="",tdLoadingTime="";
					...
					tdDname = '<td style="background-color:'+GetColor(Dstatus[positions[i]],Pspeed[positions[i]],currentStatus)+'"><a href="#" style="text-align:center;color:#000;" onClick=ShowPopUp('+positions[i]+') ><strong><h4>'+Dname[positions[i]]+'</h4></strong></a></td>';
                 ...
                 //   tdTripsOut= '<td style="text-align:center;">'+DtripsOut[positions[i]]+'</td>';
					...
					txt = ;
