...
class Form
{
	 ...
	 */
	public function val($typeOfValidator, $arg = null , $msg = null)
	{
		if ($arg == null)
		$error = $this->_val->{$typeOfValidator}($this->_postData[$this->_currentItem],$msg);
		else
		$error = $this->_val->{$typeOfValidator}($this->_postData[$this->_currentItem], $arg,$msg);
		...
		public function compare($fieldOne, $fieldTwo , $msg = null)
	{
		//echo "called ";
		$error = $this->_val->compare($_POST[$fieldOne],$_POST[$fieldTwo],$msg);
		
		if ($error)
		$this->_error[$fieldOne] = $error;
